...
export const Input = forwardRef<HTMLInputElement, InputHTMLAttributes<HTMLInputElement>>(
  ({ className, ...props }, ref) => (
    <input
      ref={ref}
      className={cn(
        "h-10 w-full rounded-xl px-3 text-sm border border-[var(--color-neutral-300)] bg-white",
...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaHTMLAttributes<HTMLTextAreaElement>>(
  ({ className, ...props }, ref) => (
    <textarea
      ref={ref}
      className={cn(
        "w-full rounded-xl px-3 py-2 text-sm border border-[var(--color-neutral-300)] bg-white",
        "placeholder:text-[var(--color-neutral-400)] focus:outline-none focus:border-black",
        className
      )}
      {...props}
